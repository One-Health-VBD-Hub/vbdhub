...
function collectDatasetFiles(objects: HubBucketObject[], prefix: string): HubDatasetFile[] {
  return objects
...
async function processDataset(
  ...
  const stream = await storage.getObjectStream({ objectKey: dataset.object.key });
